...
class Captcha(View):
    ...
    @nextcord.ui.button(label='Verify Me!', style=nextcord.ButtonStyle.green)       # button decorator
...
class button5(Button):
    ...
    async def callback(self, interaction):
        ...
        if self.label == e:
            self.style = nextcord.ButtonStyle.green
            role = get(interaction.guild.roles, id=953329228185763951) or get(interaction.guild.roles, id=951775555584393256)
            emb = nextcord.Embed(
            ...
            # await interaction.followup.send(content="https://tenor.com/view/tnrdiscord-tnrwelcome-the-new-resistance-new-resistance-nft-tnr-game-gif-23943155", ephemeral=True)
            time.sleep(5)
...
class Layout(View):
    def __init__(self):
        ...
        file = open(f"captchaFolder/captcha_{ui}.txt", "r")
        a = file.read(1)
        b = file.read(1)
        c = file.read(1)
        d = file.read(1)
